...
void Stencils::RHSStencil::apply(FlowField& flowField, int i, int j)
{
    ...
    const RealType* fgh_xdir = flowField.getFGH().getVector(i-1,j);
    const RealType* fgh_ydir = flowField.getFGH().getVector(i, j-1);

    value = (((fgh[0]-fgh_xdir[0])/dx) + ((fgh[1]-fgh_ydir[1])/dy))/dt;
}
...
void Stencils::RHSStencil::apply(FlowField& flowField, int i, int j, int k)
{
    ...
    const RealType* fgh_xdir = flowField.getFGH().getVector(i-1, j, k);
    const RealType* fgh_ydir = flowField.getFGH().getVector(i, j-1, k);
    const RealType* fgh_zdir = flowField.getFGH().getVector(i, j, k-1);

    value = (((fgh[0]-fgh_xdir[0])/dx) + ((fgh[1]-fgh_ydir[1])/dy) + ((fgh[2]-fgh_zdir[2])/dz))/dt;
}