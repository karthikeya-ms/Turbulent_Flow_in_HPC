...
void Stencils::ViscosityStencil::apply(TurbulentFlowField& flowField, int i, int j) {
  RealType delta = 0.0;
  if (parameters_.turbulent.delta == 0) { //Case for no boundary layer
    delta = 0.0;
  } 
  else if (parameters_.turbulent.delta == 1) { //Case for laminar flat plate
  ...
  else if (parameters_.turbulent.delta == 2) {//Case for turbulent flat plate
    auto x = parameters_.meshsize->getPosX(i, j) + 0.5 * parameters_.meshsize->getDx(i, j);
    delta  = 0.382 * x / pow(parameters_.walls.vectorLeft[0] * x * parameters_.flow.Re, 0.2);
  }

  //Calculate mixing length by Prandtl's model
  RealType lm = std::min(parameters_.turbulent.k * flowField.getheight().getScalar(i, j), 0.09 * delta);
  ...
  flowField.getVis().getScalar(i, j) = lm * lm * sqrt(2 * SijSij);
}

void Stencils::ViscosityStencil::apply(TurbulentFlowField& flowField, int i, int j, int k) {
  RealType delta = 0.0;
  ...
  RealType lm = std::min(parameters_.turbulent.k * flowField.getheight().getScalar(i, j, k), 0.09 * delta);
  ...
  flowField.getVis().getScalar(i, j, k) = lm * lm * sqrt(2 * SijSij);
}
