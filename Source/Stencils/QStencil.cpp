...
void Stencils::QStencil::apply(TurbulentFlowField& flowField, int i, int j)
{
    ...
    if ((obstacle & OBSTACLE_SELF) == 0) { // If the cell is fluid
        ...
        const RealType fv1 = pow(X, 3) / (pow(X, 3) + pow(cv1, 3)) + EPSILON;
        const RealType fv2 = 1 - X / (1 + X * fv1) + EPSILON;
        const RealType S_ = S + fv2 * ChVis / (pow(_k, 2) * pow(d, 2));

        const RealType ft2 = ct3 * exp(ct4 * pow(X, 2) * (-1.0)) + EPSILON;

        const RealType r = ChVis / (S_ * pow(_k, 2) * pow(d, 2) + EPSILON);
        const RealType g = r + cw2 * (pow(r, 6) - r) + EPSILON;
        const RealType fw = g * pow(((1 + pow(cw3, 6)) / (pow(g, 6) + pow(cw3, 6))), 0.1667) + EPSILON;
    
        flowField.getQ().getScalar(i,j) = cb1 * S_ * ChVis * (1 - ft2) 
                                        + (cb1 * ft2 / pow(_k, 2) - cw1 * fw) * pow(ChVis / d, 2) + EPSILON;
